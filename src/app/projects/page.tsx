...
export default function Projects() {
  return (
    <div className="max-w-3xl mx-auto px-5 py-12 sm:py-20">
      ...
      <p className="text-sm sm:text-base text-[var(--color-muted)] mb-6 max-w-lg ml-0 sm:ml-5">
        A pile of things I&apos;ve added to fill the space, ignoring is strongly recommended.
      </p>

      {/* The disclaimer — reusing the site's "low power" badge idiom */}
      <div className="mb-8 sm:mb-12 ml-0 sm:ml-5 max-w-lg rounded-lg border border-dashed border-[var(--color-border)] bg-[var(--color-surface)]/40 px-4 py-3 flex items-start gap-3">
        <span className="text-[var(--color-accent)] mt-0.5 shrink-0">⚠</span>
        <p className="text-xs text-[var(--color-muted)] leading-relaxed">
          <span className="font-semibold text-[var(--color-foreground)]">
            these are placeholders.
          </span>{" "}
          the real stuff is loading. or it isn&apos;t. honestly the bar is just
          &ldquo;eventually, possibly, if the wifi holds.&rdquo; if you need
          actual{" "}
          <a
            href="/#ok-but-actually"
            className="text-[var(--color-primary)] underline decoration-dotted underline-offset-2 hover:text-[var(--color-accent)] transition-colors"
          >
            closure
          </a>
          , the collapsible on the home page has the grown-up version of this
          sentence.
        </p>
      </div>
      ...
      <p className="text-xs text-[var(--color-muted)] mt-10 font-mono">
        ps — demos are fake, and the images(if any) were
        generated by an AI that was very confident and slightly wrong. The
        code, however, mostly doesn't exist. I am working on new ones, maybe they show up eventually when Windows market share drops below 5%.
      </p>
    </div>
  );
}
